...
const STEPS = [
  { id: 0, title: "Имя и возраст", icon: UserIcon },
  { id: 1, title: "Стадия", icon: Rocket },
  { id: 2, title: "Формат бизнеса", icon: Layers },
  { id: 3, title: "Ресурсы", icon: Wallet },
  { id: 4, title: "Цель на 30 дней", icon: Target },
  { id: 5, title: "Итог", icon: Sparkles },
];
...
const STAGE_OPTIONS: { value: Stage; label: string; desc: string; emoji: string }[] = [
  ...
  { value: "mvp", label: "Есть MVP", desc: "Сделал первые попытки запуска", emoji: "🚀" },
  { value: "selling", label: "Уже продаю", desc: "Бизнес работает, нужно улучшить", emoji: "💰" },
];
...
export default function OnboardingPage() {
  ...
  const handleFinish = () => {
    ...
    setProfile({
      ...
      goal: data.goal,
      track: rec.track,
      hasDiagnostic: data.stage === "selling" || data.stage === "mvp",
  ...
  return (
    <div className="min-h-[100dvh] bg-bg">
      <div className="max-w-[560px] mx-auto px-5 pt-6 pb-32">
        {/* Progress dots */}
        <div className="flex items-center justify-between mb-8">
          <button
            onClick={() => (step === 0 ? router.push("/") : setStep((s) => s - 1))}
            aria-label="Назад"
            className="w-10 h-10 rounded-full hover:bg-bg-muted flex items-center justify-center"
          >
          ...
          >
            ...
            {step === 1 && (
              <StepShell
                ...
                subtitle="От этого зависит, с чего начнём"
              >
                ...
                {data.stage === "selling" && (
                  <p className="text-xs text-secondary mt-3 bg-bg-muted/50 rounded-xl p-3">
                    На стадии «уже продаю» добавим быструю диагностику — она может зачесть часть базовых блоков.
                  </p>
                )}
              </StepShell>
            )}
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        data.format === opt.value
                          ...
                          : "border-bg-muted bg-bg-surface hover:border-bg-muted",
                      )}
                    >
            ...
            {step === 3 && (
              <StepShell
                ...
                subtitle="Будем честны — это влияет на старт"
              >
      ...
      {/* Sticky footer */}
      <div className="fixed bottom-0 inset-x-0 bg-bg-surface/95 backdrop-blur-md border-t border-bg-muted pb-[env(safe-area-inset-bottom)]">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "border-primary bg-primary-soft" : "border-bg-muted bg-bg-surface hover:border-bg-muted",
      )}
    >
...
function SummaryStep({ data }: { data: Record<string, unknown> }) {
  ...
  const trackName: Record<Track, string> = {
    ...
    regular: "Обычный бизнес",
  };

  const startModuleLabel = rec.startModuleLabel;
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-ink leading-tight">Вот ваш план</h1>
      <p className="text-secondary mt-1.5 mb-6">AI-кофаундер подобрал стартовую точку</p>
      ...
      <div className="card p-5 mb-4 bg-gradient-to-br from-primary-soft to-bg-surface">
        ...
        <p className="font-semibold text-ink leading-relaxed">{rec.aiText}</p>
      </div>

      <div className="space-y-3">
        <SummaryRow label="Трек развития" value={trackName[rec.track]} emoji="🧭" />
        <SummaryRow label="Стартовый блок" value={startModuleLabel} emoji="🚀" />
        <SummaryRow
          label="С учётом диагностики"
          value={rec.useDiagnostic ? "Да, часть блоков зачтена" : "Нет, проходим все блоки"}
          emoji="🔍"
        />
      </div>
    </div>
